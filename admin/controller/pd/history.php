...
class ControllerPdHistory extends Controller {
	...
	public function get_blance($customer_id){
		$this -> load -> model('pd/registercustom');
		$blance_coinmax = $this -> model_pd_registercustom -> get_wallet_coinmax_buy_customer_id($customer_id);
		return $blance_coinmax['amount'];
	}
	...
	public function load_date(){
		...
		?>
			<tr>
				<td><?php echo $i++;?></td>
				<td><?php echo $value['username'];?></td>
				<td><?php echo $get_filled_by_id['sum_filled']/100000000;?> BTC</td>
				<td><?php echo number_format($this->get_blance($value['customer_id']));?></td>
