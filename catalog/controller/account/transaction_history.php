...
class ControllerAccountTransactionHistory extends Controller {
	...
	public function matching() {
		...
		$ts_history = $this -> model_account_customer -> getTotalHistory_matching($this -> session -> data['customer_id']);
		...
		$pagination -> num_links = 5;
		$pagination -> text = 'text';
		$pagination -> url = HTTPS_SERVER . 'matching-profit.html&p={page}';
		$data['histotys'] = $this -> model_account_customer -> getTransctionHistory_matching($this -> session -> data['customer_id'], $limit, $start);

		$data['pagination'] = $pagination -> render();


        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/transaction_history_reffernal.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/transaction_history_matching.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/account/transaction_history_matching.tpl', $data));
        }
	}
